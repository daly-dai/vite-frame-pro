  ...
  const walkAndCopy = (treeNode: any, depth = 1) => {
    if (filter(treeNode)) {
      ...
      if (treeNode[children]) {
        (copy as any)[children] = [];

        (treeNode[children] as any[]).forEach((node: any) => {
          const subTree = walkAndCopy(node, depth + 1);
          subTree && (copy as any)[children].push(subTree);
        });
      }
